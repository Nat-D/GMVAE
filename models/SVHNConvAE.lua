...
function Model:CreateRecogniser(input_size, hidden_size, x_size, w_size, number_of_mixtures)
  local height = input_size[1] -- 32x32 for cifar
  ...
  local input = - nn.Identity()
	local hidden = input
		  ...
		  - nn.SpatialConvolution(nChannels, nFilters, 4,4,2,2,1,1)
		  - nn.SpatialBatchNormalization(nFilters)
		  - nn.ReLU(true)

          - nn.SpatialConvolution(nFilters, 2*nFilters, 4,4,2,2,1,1)
          ...
          - nn.SpatialConvolution(4*nFilters, hidden_size, 4,4)
...
function Model:CreateYGenerator(input_size, hidden_size, output_size, continous)
  ...
  local nFilters = self.nFilters
	...
	local hidden = input
          ...
          - nn.SpatialFullConvolution(hidden_size, 4*nFilters, 4,4)
          - nn.SpatialBatchNormalization(4*nFilters)
          - nn.ReLU(true)
          - nn.SpatialFullConvolution(4*nFilters, 2*nFilters, 4,4,2,2,1,1)
          - nn.SpatialBatchNormalization( 2*nFilters)
          - nn.ReLU(true)
          - nn.SpatialFullConvolution(2*nFilters, nFilters, 4,4,2,2,1,1)
          - nn.SpatialBatchNormalization( nFilters)
          - nn.ReLU(true)
          - nn.SpatialFullConvolution( nFilters, nChannels, 4,4,2,2,1,1)
          - nn.View(-1, nChannels, height, width)
